...
const COR = {
  usuario:      { accent: "#8B1A1A", lt: "#FFF0F0", border: "#FFCCCC", bg: "#FAFAF8" },
  profissional: { accent: "#1A4A8A", lt: "#EBF2FC", border: "#A8C4E8", bg: "#F0F5FA" },
};
  ...
  if (tipoConta === null && carregando) {
    return (
      <View style={[s.root, { backgroundColor: "#FAFAF8" }]}>
        <ActivityIndicator size="large" color={GREY} style={{ marginTop: 60 }} />
      </View>
    );
  }
        ...
        <View style={s.cardContent}>
          ...
          <View style={s.cardTop}>
            <View style={s.servicoRow}>
              <MaterialCommunityIcons name="content-cut" size={18} color={CL.accent} />
          ...
          <View style={s.infoRow}>
            <MaterialCommunityIcons name="account-tie" size={15} color={CL.accent} />
            <Text style={s.infoTexto}>{"  "}Profissional: </Text>
            <View style={[s.profBadge, { backgroundColor: CL.lt, borderColor: CL.accent }]}>
              <Text style={[s.profBadgeTexto, { color: CL.accent }]}>{item.profissionalNome || "—"}</Text>
            </View>
          </View>
          ...
          {confirmando && (
            <View style={[s.confirmaBox, { backgroundColor: CL.lt, borderColor: CL.border }]}>
              <MaterialCommunityIcons name="alert-circle-outline" size={15} color={CL.accent} />
              <Text style={[s.confirmaTexto, { color: CL.accent }]}>  Remover este agendamento?</Text>
              <TouchableOpacity style={[s.confirmaBtnSim, { backgroundColor: CL.accent }]} onPress={() => executarDelete(item.id)}>
                <Text style={s.confirmaBtnSimTexto}>Sim</Text>
              </TouchableOpacity>
              <TouchableOpacity style={s.confirmaBtnNao} onPress={() => setConfirmandoId(null)}>
      ...
      {carregando ? (
        ...
        <View style={s.centro}>
          ...
          <Text style={s.vazioTitulo}>
            {isProfissional ? "Nenhum agendamento para você ainda" : "Nenhum agendamento ainda"}
          </Text>
          <Text style={s.vazioSub}>
            {isProfissional ? "Aguarde seus clientes agendarem." : "Vá em \"Agendar\" para criar um novo."}
          </Text>
          {!isProfissional && (
            <TouchableOpacity
              style={[s.botaoNovo, { backgroundColor: CL.accent, shadowColor: CL.accent }]}
        ...
        <>
          {isProfissional && (
            <View style={[s.filtroAviso, { backgroundColor: CL.lt }]}>
              <MaterialCommunityIcons name="filter-check" size={15} color={CL.accent} />
              <Text style={[s.filtroTexto, { color: CL.accent }]}>  Exibindo apenas seus agendamentos</Text>
            </View>
          )}
          <FlatList
            data={agendamentos}
...
const s = StyleSheet.create({
  ...
  botaoNovo:   { flexDirection: "row", alignItems: "center", paddingVertical: 12, paddingHorizontal: 24, borderRadius: 30, marginTop: 8, shadowOpacity: 0.35, shadowRadius: 8, elevation: 5 },
  botaoNovoTexto: { color: WHITE, fontWeight: "900", fontSize: 14 },

  filtroAviso: { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: "#E0E8F0" },
  filtroTexto: { fontSize: 13, fontWeight: "700" },

  lista: { padding: 16, paddingBottom: 40, gap: 12 },

  card: { backgroundColor: WHITE, borderRadius: 14, flexDirection: "row", overflow: "hidden", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 6, elevation: 3 },
  cardBar:     { width: 5 },
  ...
  profBadge:      { borderRadius: 10, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1 },
  profBadgeTexto: { fontSize: 12, fontWeight: "800" },

  badgeRow: { flexDirection: "row", marginTop: 6 },
  badge:    { flexDirection: "row", alignItems: "center", backgroundColor: GREEN, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  badgeTexto: { fontSize: 11, fontWeight: "800", color: WHITE },

  confirmaBox:         { flexDirection: "row", alignItems: "center", borderWidth: 1, borderRadius: 8, padding: 10, marginTop: 8, gap: 6 },
  confirmaTexto:       { flex: 1, fontSize: 13, fontWeight: "600" },
  confirmaBtnSim:      { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 6 },
  confirmaBtnSimTexto: { color: WHITE, fontWeight: "800", fontSize: 13 },
  confirmaBtnNao:      { backgroundColor: "#EEE", paddingHorizontal: 14, paddingVertical: 6, borderRadius: 6 },
