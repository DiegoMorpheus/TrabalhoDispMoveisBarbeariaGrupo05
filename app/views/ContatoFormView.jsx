...
const COR = {
  usuario:      { accent: "#8B1A1A", lt: "#F5EDE2", bg: "#FAFAF8", erroBg: "#FFF0F0", erroBorder: "#FFCCCC" },
  profissional: { accent: "#1A4A8A", lt: "#EBF2FC", bg: "#F0F5FA", erroBg: "#EBF2FC", erroBorder: "#A8C4E8" },
};
  ...
  async function salvar() {
    ...
    try {
      const contato = new ContatoEntity(
        idContato, nome, email, telefone,
        null, favorito, categoria, genero ?? "M"
      );
      ...
      if (sessao?.id) {
        ...
        await AsyncStorage.setItem(chavePerfil(sessao.id), JSON.stringify(perfil));

        if (isProfissional && sessao.nome) {
          await AsyncStorage.setItem(`genero_prof_${sessao.nome}`, genero ?? "M");
        }
      }
      ...
      >
        <View style={s.card}>
          ...
          <View style={s.cardBody}>
            ...
            {sessao?.id && (
              <>
                ...
                <View style={s.inputReadOnly}>
                  <MaterialCommunityIcons name="identifier" size={18} color={CL.accent} style={{ marginRight: 8 }} />
            ...
            {/* Erro */}
            {erro ? (
              <View style={[s.erroBox, { backgroundColor: CL.erroBg, borderColor: CL.erroBorder }]}>
            ...
            <TextInput style={s.input} placeholder="Seu nome completo" placeholderTextColor={GREY} value={nome} onChangeText={setNome} returnKeyType="next" />

            {/* E-mail */}
            <Text style={[s.label, { color: CL.accent }]}>E-MAIL</Text>
            <TextInput style={s.input} placeholder="seu@email.com" placeholderTextColor={GREY} value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" returnKeyType="next" />

            {/* Telefone */}
            <Text style={[s.label, { color: CL.accent }]}>TELEFONE</Text>
            <TextInput style={s.input} placeholder="(31) 99999-0000" placeholderTextColor={GREY} value={telefone} onChangeText={setTelefone} keyboardType="phone-pad" returnKeyType="done" />
            ...
            {/* Botão Salvar */}
            <TouchableOpacity
              style={[s.botao, { backgroundColor: CL.accent, shadowColor: CL.accent }, salvando && s.botaoDesativado]}
              onPress={salvar}
              disabled={salvando}
              activeOpacity={0.85}
            >
...
const s = StyleSheet.create({
  ...
  sucessoIcone:  { width: 90, height: 90, borderRadius: 45, justifyContent: "center", alignItems: "center" },
  ...
  cardBarra: { height: 6 },
  cardBody:  { padding: 24 },
  cardTitulo:{ fontSize: 20, fontWeight: "900", color: DARK, textAlign: "center", letterSpacing: 1.5, marginBottom: 24 },

  erroBox:   { flexDirection: "row", alignItems: "center", borderWidth: 1, borderRadius: 8, padding: 10, marginBottom: 16 },
  ...
  botao:           { borderRadius: 8, height: 52, justifyContent: "center", alignItems: "center", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.35, shadowRadius: 8, elevation: 5 },
